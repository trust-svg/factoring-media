...
- 所要時間: 入札ボタン 173ms / 入札額入力 19ms / 確認画面へ 125ms = **合計 317ms**。
  2026-08-29 の実入札(Stage 3)では確定クリックまで含めた `placeBid` 全体が **2574ms**。
  実行秒数の既定30秒前に対して十分小さい。⚠️ 内訳(確定クリック単体の所要時間)は
  計測していない。1件だけの値なので、ヤフオク側が重い時間帯の余裕は未知。
...
- ✅ **確定クリックは 2026-08-29 00:39 に実行済み**(n1242036522 / 11円 / Stage 3)。
  `placeBid` が SUCCESS を返し、読み戻したページに `<h1>あなたが最高額入札者です</h1>`
  が出た。**確定ボタンを押すと本当に入札が成立する**ことがこれで取れた。
  同時に `bidButton` → `priceInput` → `bidConfirmButton` → `bidSubmitButton` の
  4段すべてを実ページで通しているので、入札フロー自体は「未検証」ではなくなった。
  ⚠️ ただし通したのは **1件だけ**。UI が変われば同じ順路が壊れるので、
  selectors.ts 冒頭の表と `probeSafety.ts` の確定前ガードは維持すること。
  ⚠️ 自動入札なので **現在価格は 1円のまま**成立した(上限額だけが11円になる)。
  「現在価格が入札額まで上がっていれば成功」という読み方は誤り。
...
残る未検証: **自分が落札した側の入札履歴**(2026-08-29 の1件は 21円で競り負けた。
WON 側の陽性対照は他人の行=`ymb******** … 最高額入札者` でしか見ていない)、
