...
実装時の判断: 以下プレビューは設計の網羅性確認用。実際の CLAUDE.md には **3列の簡潔テーブル（種別/場所/内容）に簡略化**して追加する（リスク3「Map の肥大化で CLAUDE.md が膨張」を防ぐため）。Stock/Flow/外部SSoT の意味分けは spec 本ファイル § 2 を参照する形に委譲する。
