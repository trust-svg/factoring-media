...
// | watchlistItemLink      | 🚨 不合格 | 当たるが **おすすめカルーセルまで拾う**(下の地雷9)。スコープが要る |
// | watchlistNextPage      | ❌ 全滅   | `/my/watchlist` に「次へ」が無い。ページャではない可能性(地雷9) |
//
...
//    ウォッチしていない商品が予約候補に並ぶ。
//    → 件数だけで合格にしない。同じページを2回読んで件数が一致することを
//      条件にする(判定は pageReady.ts の listStabilityVerdict、
//      所属ブロックの内訳はプローブの ancestryReport が出す)。
//    ついでに: `/my/watchlist` に「次へ」リンクは無い。ページャではなく
//    スクロール追加読み込みか、「終了分」タブ切り替えの可能性が高い。
//    watchlistNextPage を諦める前に、どちらなのかを実ページで確かめること。
// =============================================================
