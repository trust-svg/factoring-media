...
_KOBUTSU_RULES: list[tuple[str, list[str]]] = [
    (
        "金券類",
        [
            "商品券",
    ...
    (
        "自動車・自動二輪類",
        ["バイク", "オートバイ", "スクーター", "自動車", "オートバイ"],
    ),
    (
        "自転車類",
        ["自転車", "ロードバイク", "クロスバイク", "マウンテンバイク", "電動アシスト"],
    ),
    (
        "時計・宝飾品類",
        [
...
def guess_kobutsu_category(title: str) -> str:
    """商品名から古物区分を推定する（該当なし → ''）"""
    tl = title.lower()
    for cat, keywords in _KOBUTSU_RULES:
