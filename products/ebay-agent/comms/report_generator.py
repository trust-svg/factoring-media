...
def _calc_kpi(records: list[SalesRecord]) -> dict:
    if not records:
        return {
            ...
            "avg_profit_per_order_usd": 0,
            "avg_margin_pct": 0,
            "total_source_cost_jpy": 0,
            "total_ebay_fees_usd": 0,
            "total_payoneer_fees_usd": 0,
        }
...
def _top_products(
    records: list[SalesRecord], limit: int = 10, worst: bool = False
) -> list[dict]:
...
def _inventory_analysis(db: Session) -> dict:
    ...
    return {
        ...
        "out_of_stock_rate_pct": round(out_of_stock / total_listings * 100, 1)
        if total_listings
        else 0,
        "dead_stock_count": 0,
        "dead_stock_items": [],
...
def _category_breakdown(records: list[SalesRecord], db: Session) -> list[dict]:
    ...
    cat_map: dict[str, dict] = defaultdict(
        lambda: {"orders": 0, "revenue": 0, "profit": 0}
    )
    for r in records:
...
def _price_competitiveness(db: Session, days: int = 7) -> dict:
    ...
    cheaper = sum(
        1 for r in records if r.our_price_usd < r.lowest_competitor_price_usd * 0.95
    )
    expensive = sum(
        1 for r in records if r.our_price_usd > r.avg_competitor_price_usd * 1.10
    )
...
def _generate_suggestions(
    kpi: dict, inventory: dict, procurement: dict, comparison: dict
) -> list[dict]:
    """データに基づくAI改善提案"""
    suggestions = []
...
def generate_report(report_type: str, target_date: datetime | None = None) -> dict:
    ...
    try:
        ...
        if existing:
            logger.info(
                f"Report {report_type} {period_label} already exists (id={existing.id})"
            )
        ...
        price_comp = _price_competitiveness(
            db, days=7 if report_type == "weekly" else 30
        )
