"""ローカルMacでYahoo落札履歴をスクレイプしてVPSに取込む

VPS IPがYahooにEEA/UK扱いされるため、ローカルMac（日本住宅IP）で
スクレイプしてからVPS APIに直接POST する。

使い方（products/ebay-agent/ から実行）:
    python3 scripts/scrape_yahoo_local.py
...
async def main() -> None:
    ...
    print("Yahoo落札履歴をスクレイプ中（ローカルMac実行）...")
    try:
        ...
        if "LOGIN_REQUIRED" in str(e):
            print("❌ Yahooセッション切れ。")
            print(
                "   python3 scripts/login_and_save_cookies.py yahoo を再実行してください"
            )
            sys.exit(1)
        raise
    ...
    req = urllib.request.Request(
        f"{VPS_URL}/api/procurements/yahoo-local-import",
        data=body,
        headers={
