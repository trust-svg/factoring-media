"""ローカルMacでYahooフリマ購入履歴をスクレイプしてVPSに取込む"""
...
async def main() -> None:
    ...
    print("Yahooフリマ購入履歴をスクレイプ中（ローカルMac実行）...")
    try:
        ...
        if "LOGIN_REQUIRED" in str(e):
            print(
                "❌ Yahooフリマセッション切れ。ブラウザが開かない場合は再ログインしてください"
            )
            sys.exit(1)
        raise
    ...
    req = urllib.request.Request(
        f"{VPS_URL}/api/procurements/yahoo-flea-local-import",
        data=body,
