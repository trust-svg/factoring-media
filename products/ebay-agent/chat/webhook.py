...
def _parse_xml_notification(body: str) -> Optional[Dict]:
    ...
    if event_type == "feedback_received":
        ...
        extra_data["transaction_price"] = (
            _find_text(root, ".//e:TransactionPrice", ns) or ""
        )
        extra_data["quantity_purchased"] = (
            _find_text(root, ".//e:QuantityPurchased", ns) or ""
        )
        extra_data["item_title"] = _find_text(root, ".//e:Title", ns) or ""

    elif event_type == "item_shipped":
        extra_data["tracking_number"] = (
            _find_text(root, ".//e:ShipmentTrackingNumber", ns) or ""
        )
...
async def handle_notification(body: str) -> Dict:
    ...
    if event_type in auto_message_events and buyer:
        ...
        # リピート購入エンジン Phase 1: ポジティブ Feedback を D7 キューに積む
        # comment_type は eBay XML で "Positive" / "Neutral" / "Negative"
        if event_type == "feedback_received":
            try:
                from chat.repeat_engine import enqueue_post_feedback

                rating = (data.get("comment_type") or "").strip()
                comment = data.get("comment_text") or ""
                queue_result = enqueue_post_feedback(
                    buyer_username=buyer,
                    order_id=order_id,
                    item_id=item_id,
                ...
                result["repeat_engine"] = queue_result
            except Exception as e:
                logger.warning(f"repeat_engine enqueue error: {e}")
                result["repeat_engine_error"] = str(e)
