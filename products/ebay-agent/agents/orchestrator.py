...
def _cached_system() -> list[dict]:
    """system プロンプトを 1h prompt cache 付きで返す。"""
    return [
        {
            "type": "text",
...
def _cached_tools() -> list[dict]:
    """AGENT_TOOLS の最後のツールに cache_control を付与して全ツール定義をキャッシュ対象にする。"""
    if not AGENT_TOOLS:
        return AGENT_TOOLS
    cached = list(AGENT_TOOLS)
    last = dict(cached[-1])
...
async def run_agent(user_message: str) -> dict:
    ...
    while response.stop_reason == "tool_use" and iteration < MAX_AGENT_ITERATIONS:
        ...
        for block in response.content:
            if block.type == "tool_use":
                ...
                logger.info(
                    f"Agent tool call [{iteration}]: {tool_name}({json.dumps(tool_input, ensure_ascii=False)[:200]})"
                )
    ...
    logger.info(
        f"Agent completed: {iteration} iterations, {len(tool_call_log)} tool calls"
    )
