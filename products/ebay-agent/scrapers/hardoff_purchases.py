"""ハードオフ ネットモール 購入履歴スクレイパー

Playwrightで netmall.hardoff.co.jp の注文履歴を巡回し、
...
COOKIE_FILE = COOKIE_DIR / "hardoff_cookies.json"

ORDERS_URL = "https://netmall.hardoff.co.jp/mypage/orders/"
...
async def _save_cookies(context):
    ...
    logger.info(f"HardOff cookies saved ({len(cookies)} entries)")
...
async def _load_cookies(context):
    if COOKIE_FILE.exists():
        ...
        logger.info(f"HardOff cookies loaded ({len(cookies)} entries)")
        return True
    return False


async def scrape_hardoff_purchases(
    on_progress=None,
    headless: bool = False,
) -> list[dict]:
    """ハードオフ ネットモール注文履歴を巡回して商品情報を取得"""
    ...
    async with async_playwright() as p:
        ...
        if on_progress:
            on_progress("ハードオフ注文履歴にアクセス中...", 0, 0)
        ...
        if "login" in page.url or "signin" in page.url:
            ...
            if on_progress:
                on_progress("ハードオフにログインしてください...", 0, 0)

            try:
                await page.wait_for_url(
                    lambda url: "mypage" in url or "orders" in url,
                    timeout=300000,
                )
                await asyncio.sleep(3)
            except Exception:
        ...
        while True:
            ...
            page_orders = await page.evaluate("""() => {
                ...
                // 注文ブロックを検索（テーブル行、カード、リストアイテムなど）
                const orderBlocks = document.querySelectorAll(
                    '[class*="order"], [class*="history"], tr[class*="item"], .purchase-item, li[class*="order"]'
                );

                // フォールバック: 商品リンクから取得
                const productLinks = document.querySelectorAll('a[href*="/product/"], a[href*="/item/"]');

                const seen = new Set();

                for (const link of productLinks) {
                    const href = link.getAttribute('href') || '';
                    const idMatch = href.match(/\\/(product|item)\\/([\\w-]+)/);
                    if (!idMatch) continue;
                    ...
                    const container = link.closest('tr') || link.closest('[class*="order"]')
                        || link.closest('li') || link.closest('div[class*="item"]') || link.parentElement;
                    ...
                    const itemUrl = href.startsWith('http') ? href : 'https://netmall.hardoff.co.jp' + href;
            ...
            has_next = await page.evaluate("""() => {
                ...
                for (const a of links) {
                    ...
                    if (text === '次へ' || text === '次のページ' || text === 'Next' || text === '>' || text === '›') {
                        a.click();
                        return true;
                    }
                }
                // ページネーション番号
                const pagers = document.querySelectorAll('a[href*="page="], a[href*="p="]');
        ...
        for idx, item in enumerate(unique_items):
            ...
            try:
                ...
                try:
                    await page.wait_for_selector('img[src*="hardoff"], [class*="price"]', timeout=8000)
                except Exception:
                    pass
                ...
                if not item.get("image_url"):
                    try:
                        img_url = await page.evaluate("""() => {
                            ...
                            const img = document.querySelector('img[src*="hardoff"]');
