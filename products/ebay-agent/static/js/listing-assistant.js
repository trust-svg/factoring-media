...
function _getResearchMargin(itemId) {
  const data = _researchResults.get(itemId)
    ?? _researchResults.get(Number(itemId))
    ?? _researchResults.get(String(itemId));
  if (!data || data.status !== 'ok') return -1000;
  return data.margin_pct ?? 0;
}

function _reorderByResearch() {
  const container = document.getElementById('reorderList');
  if (!container) return;

  const items = Array.from(container.querySelectorAll('.la-reorder-item'));
  items.sort((a, b) => {
    const idA = parseInt(a.id.replace('reorder-', ''), 10);
    const idB = parseInt(b.id.replace('reorder-', ''), 10);
    return _getResearchMargin(idB) - _getResearchMargin(idA);
  });

  const researched = items.filter(el => {
    const id = parseInt(el.id.replace('reorder-', ''), 10);
    return _getResearchMargin(id) > -1000;
  }).length;

  // Replace section headers with a single summary; DOM nodes (incl. inputs) are reattached
  container.innerHTML = `<div class="la-reorder-section-header">リサーチ完了 ${researched}/${items.length}件 — 利益率順</div>`;
